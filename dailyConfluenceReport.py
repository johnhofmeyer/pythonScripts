...
def getProductionDefects():
	...
	try:
		...
		print defectRequest.url
		print defectFilter
		print defectRequest.text
		sys.exit()
	...
	for argVal in sys.argv:
		if (argVal.find("certPage") > -1):	# 
			if (argVal.find("certPage=") >-1):
				...
				return("Mediabrands+Unity+Release+"+str(determineReleaseNumber())+"+Certification")
	return (False)
...
def dbCoverageRequest(coverageQuery,projectName,sprintNumber,sprintDay):
	...
	for (passed, failed, blocked, untested) in pythonCursor:
		...
		executed=totalTests-(untested+blocked)
...
#	dailyTestStatus=requests.get("https://wiki.mbww.com/rest/api/content?title=Daily+Metrics&expand=body.view,version,extensions.ancestors", headers={'Content-Type': 'application/json'}, auth=jiraAuth)
#	dailyPageContent=dailyTestStatus.json()
#	print dailyPageContent
#	exit()
	...
	certTableHeader+='<th class="confluenceTh">(B) TC attempted</th>'
	certTableHeader+='<th class="confluenceTh"><p>(C) % TC Attempted</p><h6 id="TestCoveragePercentage"><span style="color: rgb(51,102,255);">(B/A)* 100</span></h6></th>'
	...
	certTableHeader+='<th class="confluenceTh"><p>(K) % Bugs Deferred</p><p>excluding bugs from backlog</p><h6 id="CoverageforCurrentSprintDay-((G-H)/G)*100"><span style="color: rgb(51,102,255);">((G - H)/G) * 100</span></h6></th>'
	certTableHeader+='<th class="confluenceTh"><p>(L) % Bugs Deferred</p><p>including bugs from backlog</p><h6 id="CoverageforCurrentSprintDay-((G-I)/G)*100"><span style="color: rgb(51,102,255);">((G - I)/G) * 100</span></h6></th>'
	...
	certificationPage+='<h2 id="CadreonReleaseCertification-ReleaseDates">Deployment Dates</h2>'
	certificationPage+='<p>Staging Deployment: Friday, August 3, 4:00 pm Pacific</p>'
	certificationPage+='<p>Production Deployment: Monday, August 6, 5:00 pm Pacific</p>'
	...
	confluenceTableHeader+='<th class="highlight-blue confluenceTh" colspan="6" data-highlight-colour="blue" style="text-align: center;">Automated Tests</th>' 			# Col 8-11
	...
	confluenceTableHeader+='<th class="highlight-grey confluenceTh" data-highlight-colour="grey">Feature</th>'		# Column 6
	...
	confluenceTableHeader+='<th class="highlight-blue confluenceTh" data-highlight-colour="blue">Pass | Fail</th>'
	...
	auto_query= ("SELECT sum(passed), sum(failed), sum(blocked), sum(untested) "
				  "FROM results WHERE test_type='automated' and project=%s and sprint_number=%s and sprint_day=%s;")
	...
	manRegression_query= ("SELECT sum(passed), sum(failed), sum(blocked), sum(untested) "
				  "FROM results WHERE test_type='manual' and project=%s and sprint_number=%s and sprint_day=%s;")
	regress_query= ("SELECT sum(passed), sum(failed), sum(blocked), sum(untested) "
				  "FROM results WHERE test_type != 'feature' and project=%s and sprint_number=%s and sprint_day=%s;")
	...
	dailyQAStatus=""
	featureMilestone=sprintName+" - Feature Scenarios"
	...
	if (certificationPageTitle!=False) :
		...
		certificationPage=certpage.json()
	'''
	...
	for project in testRailProjects:
		...
		if (projectID not in ("6", "10", "11", "14", "15","16","17","24","26","27","28","29","43","44","46","47", "51","52", "53", "55")): # 9 is Datorama - which has a one-to-many testrail-to-Jira relationship
																							## 43= IPI, which may be over ?!
																							## 51=ADE
																							## 55=AOI
			...
			for currentMilestone in projectMilestones:
				...
				if (currentMilestone['name'].find(featureMilestone) > -1):
					...
					## default sprintLookback=4
					while (offset<sprintLookback):
						...
						# {"passed":passed,"failed":failed,"blocked":blocked,"untested":untested,"total":totalTests,"executed":executed}
						...
						manualCoverage=getTestCoverage(manual_query,project['name'],currentSprint-offset,sprintDay)
						
						# Manual Regression Execution
						manualRegression=getTestCoverage(manRegression_query,project['name'],currentSprint-offset,sprintDay)
						...
						totalManualTests=manualCoverage['total']['today']
						totalManualExecuted=manualCoverage['executed']['today']
						totalManualExecutedYesterday=manualCoverage['executed']['yesterday']
						...
						# Manual Regression
						manRegressTotal=manualRegression['total']['today']
						manRegressExecuted=manualRegression['executed']['today']
						manRegressExecutedYesterday=manualRegression['executed']['yesterday']
						...
						totalAutoExecuted=automatedCoverage['executed']['today']
						totalAutoTests=automatedCoverage['total']['today']
						
						totalAutoPassed=automatedRegression['passed']['today']
						totalAutoFailed=automatedRegression['failed']['today']
						...
						for (jira_project) in subCursor:
							...
							if (totalTests>0):
								...
								if (offset <1 ):
									...
									bugsInBacklog=getBugsInBacklog(jirProj)
									...
									totalBacklogFilter='<a class="external-link" href="https://projects.mbww.com/issues/?jql=project%20%3D%20'+jirProj+'%20AND%20issuetype%20%3D%20Bug%20and%20status!%3DClosed" rel="nofollow">'+str(bugsInBacklog)+'</a>'
									...
									nextProjectRow='<td colspan="1" class="'+projectID+'">'+certProject+'</td>'
									
									'''
									certificationpage=getCertificationPage()
									if (nextProjectRow in certificationpage):
										print "Cert Table update"
									'''
									# cert row Columns 1-6
									certTable+=nextProjectRow
									if (totalExecuted > 0):
										certTable+='<td>'+str(totalTests)+'</td><td>'+str(totalExecuted)+'</td><td>'+'{:.2%}'.format(float(totalExecuted)/float(totalTests))+'</td><td>'+str(totalPassed)+'</td><td>'+'{:.2%}'.format(float(totalPassed)/float(totalTests))+'</td>'
									else:
										certTable+='<td>'+str(totalTests)+'</td><td> 0 </td><td>0.00%</td><td>'+str(totalPassed)+'</td><td> 0.00% </td>'
									
									# cert row Columns 7
									certTable+='<td>'+str(critical+major)+'</td>'
									
									# cert row 8-11 - Rows with project-specific filters
									certTable+='<td>'+openBugFilter+'</td>'
									certTable+='<td>'+resolvedNoBacklogFilter+'</td>'
									certTable+='<td>'+allResolvedFilter+'</td>'
									certTable+='<td>'+totalBacklogFilter+'</td>'
									
									# cert rows Columns 12,13
									if (openedBugs > 0):
									...
									# cert row Columns 14-15
									if (projectCertified) :
										#criteria='<ac:structured-macro ac:name="status"><ac:parameter ac:name="colour">Green</ac:parameter><ac:parameter ac:name="title">Yes</ac:parameter><ac:parameter ac:name="subtle">false</ac:parameter></ac:structured-macro>'
										certTable+='<td><span><ac:structured-macro ac:name="status"><ac:parameter ac:name="colour">Green</ac:parameter><ac:parameter ac:name="title">YES</ac:parameter><ac:parameter ac:name="subtle">false</ac:parameter></ac:structured-macro></span></td><td></td></tr>'
										criteria="yes"
										criteriaStyle='Green'
									else:
										criteria="NO"
										criteriaStyle='Red'
										certTable+='<td><span><ac:structured-macro ac:name="status"><ac:parameter ac:name="colour">Red</ac:parameter><ac:parameter ac:name="title">NO</ac:parameter><ac:parameter ac:name="subtle">false</ac:parameter></ac:structured-macro></span></td><td></td></tr>'
									...
									for ticket in sprintTickets['issues']:
										...
										try:
											...
											try:
												...
												print "JIRA exception - could not get ticket detail"
												sys.exit()
											...
											try:
												...
												print ticketKey," does not have a due date"
												ticketDueDate=None
											...
											print "EXCEPTION WITH JIRA TICKET"
											print ticket
									...
									for (component, versionMethod, jenkinsProject) in componentCursor:
										...
										## retry: 
										## /job/{{jenkinsProject}}/promotion/process/deploy-qa/
										...
										## Retry for some of the new projects
										if (componentQAPromotion.status_code >299):
											jenkinsAPI="/job/"+jenkinsProject+"/process/deploy-qa/api/json"
											jenkinsQABuildPromotion=jenkinsHost+jenkinsAPI
											componentQAPromotion=requests.get(jenkinsQABuildPromotion)
										...
										lastSuccessBuild=".?."
										buildLink="n/a"
										if (componentQAPromotion.status_code <300) and (jenkinsProject not in "/job/ttag/deploy-qa/api/json"):
											try:
								...
								## Regression Tests
								# Column 11 - Automated Regression passed | failed
																
								confluenceTableRow+='<td style=\"text-align: center;\">'+str(totalAutoPassed)+' | '+str(totalAutoFailed)+'</td>'
								
								# Column 12 - Automated Regression total executed
								
								confluenceTableRow+='<td style=\"text-align: center;\">'+str(totalAutoExecuted)+' / '+str(totalAutoTests)+'</td>'
								
								# Column 14 - Automated Regression %
								#autoStyle=black
					...
					print "Total Tests: ",totalTestCoverage['total']['today']
					print "Functional Tests: ",functionalCoverage['total']['today']
					print "Automated Tests: ",automatedCoverage['total']['today']
					print "Regression Tests: ",regressionCoverage['total']['today']
					print "Automated Regression: ",automatedRegression['total']['today']
	...
	confluenceGet=requests.get("https://wiki.mbww.com/rest/api/content?title=Test+Coverage+for+Current+Sprint+Day&expand=body.view,version,extensions.ancestors", headers={'Content-Type': 'application/json'}, auth=jiraAuth)
	...
	print "Updated: https://wiki.mbww.com/display/AS/Coverage+for+Current+Sprint+Day"
	
	# pass in the value certPage=PAGENAME_HERE
	certificationPageTitle=getCertificationPage()
	#certificationPageTitle="Mediabrands+Unity+Release+2.35+Certification"
	if (certificationPageTitle!=False) :
	...
	#certPage=Mediabrands+Unity+Release+2.37+Certification
